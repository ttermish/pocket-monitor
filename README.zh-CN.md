...
<p align="center">
  <img src="docs/images/phone-idle.png" width="360" alt="随身屏首页：等待画面接入，提供允许视频访问、旋转、视频设置和全屏入口">
</p>

首页提供连接入口和常用画面操作。上图由应用实际界面自动化渲染，展示未连接采集卡的状态；目前尚未完成真实 HDMI 采集验收。
...
连接多张采集卡时，可从「视频设置」中选择设备。建议第一次只连接一张，方便确认输入来源。
...
| 切换格式 | 连接后打开「视频设置」，选择设备支持的分辨率、帧率和编码 |
| 停止预览 | 点击「停止预览」，或在「视频设置」中断开连接 |
...
**一直等待画面或视频中断？** 启动 8 秒无帧时，App 最多尝试两次较低规格。仍无画面时检查连接，或在「视频设置」中手动切换格式；已经出画后的断流不会反复自动重连。
